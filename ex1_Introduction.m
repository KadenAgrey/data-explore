...
%   1. the format required to define the pushbuttons and their
%      callback functions (functions executed when they are pressed). 
...
% arguments of pushbuttons on the figure window. exploreResults will make
% space for and place the pushbuttons automatically. 
% 
% Each element of this cell array is another cell array. The first element 
% of the nested cells is the button name, the second a function handle and
% later elements aruments to the function. Keep in mind that the first 4
% arguments must be reserved for exploreResults.
myarg1 = 'Button Pressed';
PUSHBUTTONS = {{'Display', @myFunc, myarg1}, ...
               {'Another Button', @myFunc}};
% ----------------------------------------------------------------------- %

exploreResults(FIG, PUSHBUTTONS)

%% Callback Function
function myFunc(src, event, ui, slct, varargin)
% ----------------------------------------------------------------------- %
...
% Use the optional extra argument
if nargin > 4
    disp(varargin{end})
end
