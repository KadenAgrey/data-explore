...
for p = 1:length(xplr.ln)
    ui.xpl(p).xplobj = xplr.ln(p);

    % Get labels. If a label is empty use x,y,z as default.
    lab = {'x', 'y', 'z'};
    if ~isempty(ui.xpl(p).xplobj.Parent.XLabel.String)
        lab{1} = ui.xpl(p).xplobj.Parent.XLabel.String;
    end
    if ~isempty(ui.xpl(p).xplobj.Parent.YLabel.String)
        lab{2} = ui.xpl(p).xplobj.Parent.YLabel.String;
    end
    if ~isempty(ui.xpl(p).xplobj.Parent.ZLabel.String)
        lab{3} = ui.xpl(p).xplobj.Parent.ZLabel.String;
    end

    % Construct data cell
    if isempty(ui.xpl(p).xplobj.ZData)
        ui.xpl(p).data = {lab{1}, ui.xpl(p).xplobj.XData; ...
                        lab{2}, ui.xpl(p).xplobj.YData};
    else
        ui.xpl(p).data = {lab{1}, ui.xpl(p).xplobj.XData; ...
                        lab{2}, ui.xpl(p).xplobj.YData; ...
                        lab{3}, ui.xpl(p).xplobj.ZData};
    end
    ui.xpl(p).data = [ui.xpl(p).data; user_data{p}];
end

% Initialize struct to hold data tip information
dt = struct('tip', 'link');
...
for pb = 1:size(in.Results.pbtnfcn,1)
    ui.pbtn(pb).Callback = {@ pbtnCallback, ui, dt, in.Results.pbtnfcn(pb,2:end)};
end

% Assign listener to dcm 'Enable' property
% plistenE = addlistener(ui.dcm,'Enable','PostSet',{@setTipCallbacks, fig, ui, dt, slctopt, {}});

% Assign default callbacks to dcmode
dcmode = getuimode(fig, 'Exploration.Datacursor');
% Set button down function
fcn = {@lnSelectPnt, dcmode.WindowButtonDownFcn, ui, dt, opt};
dcmode.WindowButtonDownFcn = fcn;
% Set button up function
fcn = {@mvLinkedTipsButtonUp, dcmode.WindowButtonUpFcn, ui.dcm, dt};
dcmode.WindowButtonUpFcn = fcn;
% Set key press function
fcn = {@mvLinkedTipsKeyPress, dcmode.KeyPressFcn, ui.dcm, dt};
...
function [] = lnSelectPnt(fig, event, fcn, ui, dt, slctopt)
...
if ~isxplhit || ~( isAddSelType || isAddMod || isAddOption )
    return;
end
...
%     alldt = [dt.tip];
    ...
    clns = [dt.tip.DataSource]; % cursor lines
    newdt = gobjects(0);
    for ln = xlns(~s)
        n = clns == ln;
    ...
    % Update lnktips
    curdt = findobj(alldt,'Cursor',curcur); %%%%%% The only way to get the data tip created with matlabs function is to use findall(), so trying to avoid the find all call is impossible.
...
elseif slctopt.SelectionLinkAxes
    ...
    mvSrcLinkedTips(lnkdt, curdt, {ui.xpl.xplobj})
    % We don't need to move the tips point here because this is handled in 
    % the button up function. We only ensure data sources are correct.
end
